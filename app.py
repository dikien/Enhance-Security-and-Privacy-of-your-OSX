...
def run_check(config_check, last_attempt=False, quiet_fail=False):
    ...
    for test in config_check.tests:
        ...
        # required item
        if config_check.confidence == 1 and const.CHECK_REQUIRED:
            dprint("Skipping recommended tests.")

        if not(config_check.confidence != 1) or not const.CHECK_REQUIRED:
        # if 'sudo ' not in test['command'] or not const.SKIP_SUDO_TESTS:

            # print
            # print not const.CHECK_REQUIRED
            # print not(config_check.confidence != 1)
...
def dprint_settings():
    ...
    dprint("CHECK_REQUIRED: %s" % str(const.CHECK_REQUIRED))
...
def main():
    ...
    const.CHECK_REQUIRED = args['check-required']
...
def get_sys_args():
    ...
    while len(unprocessed_args) > 0:
        ...
        elif flag == '--check-required':
            args['check-required'] = True
